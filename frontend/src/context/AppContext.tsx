...
import { ensureFreshProductImages } from '../utils/imageUtils';
...
const ActionTypes = {
  ...
  REFRESH_CART_IMAGES: "REFRESH_CART_IMAGES",
};
...
const appReducer = (state: any, action: any) => {
  switch (action.type) {
    ...
    case ActionTypes.ADD_TO_CART: {
      const { product, quantity, selectedSize, selectedColor, inventoryId } =
        action.payload;

      const existingItemIndex = state.cart.items.findIndex((item: any) => {
        if (inventoryId) {
    ...
    case ActionTypes.REMOVE_FROM_CART: {
      ...
      const updatedItems = state.cart.items.filter((item: any) => {
        if (inventoryId) {
          return item.inventoryId !== inventoryId;
        }
    ...
    case ActionTypes.REFRESH_CART_IMAGES: {
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Refresh expired image URLs for cart items on mount and periodically
  useEffect(() => {
    const refreshCartImages = async () => {
      if (state.cart.items.length === 0) return;
      
      const updatedItems = await Promise.all(
        state.cart.items.map(async (item: any) => {
          const refreshedProduct = await ensureFreshProductImages(item.product);
          if (refreshedProduct !== item.product) {
            return { ...item, product: refreshedProduct };
          }
          return item;
        })
      );
      ...
      const hasUpdates = updatedItems.some((item, index) => 
        item.product !== state.cart.items[index].product
      );
      
      if (hasUpdates) {
        console.log('Refreshed expired cart images');
        dispatch({
          type: 'REFRESH_CART_IMAGES',
    ...
    const interval = setInterval(refreshCartImages, 60 * 60 * 1000);
